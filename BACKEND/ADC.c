  ...
  uint16_t analog_input = (uint16_t) ADC0->R[0];
	...
	*/
	...
	if(measured_voltage >= 0 && (double)measured_voltage < 1.1)
	{
		...
		GPIOA->PCOR |= (1<<YELLOW_LED_PIN); /* sting ledul galben */
		GPIOA->PCOR |= (1<<RED_LED_PIN); /* sting ledul rosu */
	}
	
	else if((double)measured_voltage >= 1.1 && (double)measured_voltage < 2.2)
	{	
		GPIOA->PCOR |= (1<<RED_LED_PIN); /* sting ledul rosu */
		GPIOA->PSOR |= (1<<YELLOW_LED_PIN); /* pornesc ledul galben */
		GPIOA->PSOR |= (1<<GREEN_LED_PIN); /* pornesc ledul verde */
	}
	
	else if((double)measured_voltage >= 2.2 && (double)measured_voltage <= 3.3)
	{	
		GPIOA->PSOR |= (1<<GREEN_LED_PIN); /* pornesc ledul verde */
		GPIOA->PSOR |= (1<<RED_LED_PIN); /* pornesc ledul rosu */
		GPIOA->PSOR |= (1<<YELLOW_LED_PIN); /* pornesc ledul galben */
	}
	else
	{
		GPIOA->PCOR |= (1<<GREEN_LED_PIN); /* pornesc ledul verde */
		GPIOA->PCOR |= (1<<RED_LED_PIN); /* pornesc ledul rosu */
		GPIOA->PCOR |= (1<<YELLOW_LED_PIN); /* pornesc ledul galben */
	}
